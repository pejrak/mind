...
export const Fragments = {
  ...
  actions: {
    addCollected({ commit, state }, { fragments }) {
    ...
    async memorizeFragment({ commit }, fragmentId) {
      logger.info('memorizeFragment', fragmentId)
      commit('updateFragment', {
        fragmentId,
        property: 'memorized',
        value: true,
    ...
    async save({ dispatch, getters, state }) {
      ...
      dispatch('updatedAt', state.initiatedAt)
      return success
    },
      ...
      const array = fragments
        .map(
          ({
            ...
            memorized: !!memorized,
            notes,
            owner: owner === 'none' ? payloadOwner : owner,
            path,
            privacyLevel,
            text,
            updatedAt,
