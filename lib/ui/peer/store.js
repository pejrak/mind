  ...
  actions: {
    connectMe({ commit, dispatch, getters, rootState }) {
      ...
      connection.on('error', (err) => {
        logger.warn('Peer connection error', err.message)
      })
      ...
      switch (data.signal) {
        ...
        case COLLECTION_SIGNAL: {
          const fragments = rootGetters['fragments/onPath']({
            include: (fragment) =>
              fragment.privacyLevel > 0 &&
              fragment.updatedAt > data.updatedAt,
            path,
          })
          logger.info(
            `respondTo ${sender} fragments`,
            fragments,
          )
          if (fragments.length) {
            dispatch('sendTo', {
        ...
        case SYNC_SIGNAL: {
          logger.info('SYNC_SIGNAL', sender)
          break
        }
        default: {
    ...
    syncConnection({ dispatch }, recipient) {
      dispatch('sendTo', {
        recipient,
        message: 'Hi',
        signal: SYNC_SIGNAL,
