...
import { fragmentIsValid } from '../fragments/fragmentIsValid'
...
export const PeerStore = {
  ...
  actions: {
    connectMe({ commit, dispatch, getters, rootState }) {
      ...
      connection.on('error', () => {
        logger.warn('Peer connection error')
      })
    ...
    respondTo({ dispatch, rootGetters }, { sender, path, ...data }) {
      ...
      switch (data.signal) {
        case COLLECTION_RESPONSE_SIGNAL: {
          const fragments = data.fragments.filter(fragmentIsValid)
          logger.info(`response incoming`, fragments)
          break
        }
        case COLLECTION_SIGNAL: {
          const fragments = rootGetters['fragments/onPath']({
            include: (fragment) =>
              fragment.memorized && fragment.privacyLevel > 0,
            path,
          })

          logger.warn('respondTo', fragments)
          dispatch('sendTo', {
            recipient: sender,
